function [x, P, weight, J] = CorrMapPrun(map, para)
    ...
    % After the purning and merging step is performed, too small weight is ignored again for reducing the complexity
    
    p_ind = 0;
    map_Cov = zeros(map.J,1);
    for j = 1:map.J
        map_Cov(j,1) = sum(diag(map.P(:,:,j)));
    end
    J_set = find( (map.weight > para.pruning_T).*(map_Cov < para.pruning_COV) ==1);
    if numel(J_set)>0
        while (numel(J_set) ~= 0)
            p_ind = p_ind+1;
            ...
            for i_ind = 1:numel(J_set)
                if ( (map.x(J_set(i_ind),:) - map.x(j_ind,:) )*inv(map.P(:,:,J_set(i_ind)))*(map.x(J_set(i_ind),:) - map.x(j_ind,:) )' <=para.pruning_U)
